...
function dependenciesinstall(){
    if [ "$packagemanagertype" = "a" ]; then
        ...
        apt-get -yq install wget gawk grep sed git fail2ban swig gettext autoconf automake make libtool perl cpio xmlto asciidoc cron net-tools dnsutils rng-tools libc-ares-dev libev-dev libssl-dev zlib1g-dev libpcre3-dev libevent-dev build-essential python-dev python-pip python-setuptools python-m2crypto
...
function sslibevinstall(){
	...
	if [ "$1" = "install" ]; then
        ...
        wget -t 3 -T 30 -nv -O libsodium-${libsodiumver}.tar.gz https://github.com/jedisct1/libsodium/archive/${libsodiumver}.tar.gz
